...
const AdditionalCharges = ({ navigation, route }) => {
  const { onSubmit, additionalCharges, photoUri } = route.params || {};

  const [tax, setTax] = useState(additionalCharges?.tax?.toString() || "");
  const [tip, setTip] = useState(additionalCharges?.tip?.toString() || "");
  const [misc, setMisc] = useState(additionalCharges?.misc?.toString() || "");
  const [errorMessage, setErrorMessage] = useState(null);

  // State variable to hold the bottom offset for the Done button
  const [bottomOffset, setBottomOffset] = useState(40); // default value when keyboard is hidden

  useEffect(() => {
    if (additionalCharges) {
      setTax(additionalCharges.tax?.toString() || "");
      setTip(additionalCharges.tip?.toString() || "");
      setMisc(additionalCharges.misc?.toString() || "");
    }
  }, [additionalCharges]);

  // Use "keyboardWillShow" and "keyboardWillHide" on iOS for immediate updates
  useEffect(() => {
    const showEvent = Platform.OS === "ios" ? "keyboardWillShow" : "keyboardDidShow";
    const hideEvent = Platform.OS === "ios" ? "keyboardWillHide" : "keyboardDidHide";

    const onKeyboardShow = (e) => {
      setBottomOffset(e.endCoordinates.height + 10); // position button 10 pts above keyboard
    };
    const onKeyboardHide = () => {
      setBottomOffset(40); // reset to default when keyboard is hidden
    };
  ...
  // Photo preview overlay handlers (unchanged)
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
            ...
            <View style={styles.contentContainer}>
              ...
              <View style={styles.inputWrapper}>
                <View style={styles.inputWithPrefix}>
                  ...
                  <TextInput
                    style={[styles.phoneInput, Platform.OS === "ios" && styles.iosInput]}
                    value={tax}
      ...
      <View style={[styles.doneButtonContainer, { bottom: bottomOffset }]}>
        <TouchableOpacity style={[styles.button, styles.buttonEnabled]} onPress={handleSubmit}>
